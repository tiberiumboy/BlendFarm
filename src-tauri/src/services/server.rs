...
pub struct Server {
    tx: Sender<ToNetwork>,
    pub rx_recv: Arc<Receiver<FromNetwork>>,
    task: NodeTask,
    addr: SocketAddr,
}
...
impl Server {
    ...
    pub fn new(public_addr: SocketAddr) -> Server {
    ...
    pub fn send_file(&self, file: impl AsRef<Path>) {
        ...
        if let Err(e) = self
            .tx
            .send(NetMessage::SendFile(file_name.to_owned(), data))
        {
    ...
    pub fn send_file_to_target(&self, file_path: &PathBuf, target: Endpoint) {
        if let Err(e) = self
            .tx
            .send(NetMessage::Stream::SendFile(file_path.to_owned(), Target(target)))
        {
            println!("Failed to send file request to server! {e}");
        }
    }
    ...
    pub fn connect_peer(&self, socket: SocketAddr ) {
        self.tx.send(NetMessage::AddPeer { socket }).unwrap();
    }
}
