...
pub trait BlendFarm {
    ...
    async fn handle_inbound_request(&mut self, client: &mut NetworkController, request: String, channel: ResponseChannel<FileResponse>) {
        let (sender, receiver) = oneshot::channel();
        let cmd = FileCommand::RequestFilePath { keyword: request, sender };
        ...
        if let Some(path) = receiver.await.expect("Sender should not be dropped") {
            ...
            eprintln!("This local service does not have any matching request providing! Do something about the ResponseChannel?");
        }
    }
}
