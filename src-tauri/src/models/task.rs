...
use sqlx::{sqlite::SqliteRow, Decode, Encode, FromRow, Row};
use std::{
    ops::Range,
    path::PathBuf,
    sync::{Arc, RwLock},
...
// suggest ot try without lifetime?
impl FromRow for Task {
    fn from_row<T>(row: &T) -> Result<Self, sqlx::Error>
    where
        T: Row,
    {
        let id = Uuid::from_str(&row.try_get("id")?)?;
