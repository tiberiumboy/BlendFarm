...
impl Portal {
    ...
    pub fn new(download_path: PathBuf, cache: &mut PageCache) -> Self {
        // todo: find a way to load information here.
        let list = Self::fetch(cache).unwrap_or(Vec::new());
        Portal { 
            list,
            download_path
        }
    ...
    fn fetch(cache: &mut PageCache) -> Result<Vec<BlenderCategory>, ManagerError> {
        ...
        let mut list = iter
            .map(|c| c.extract())
            .fold(Vec::new(), |mut map: Vec<BlenderCategory>, (_, [url, major, minor])| {
                ...
                // in theory all of the blender category state should be loaded...?
                let category = BlenderCategory::new(url, major, minor, cache);
    ...
    fn get_blender_state_by_version(&self, version: &Version) -> Option<&BlenderCategoryState> {
        self.list.iter().fold(None, |result, item| {
    ...
    pub fn fetch_blender(&mut self, version: &Version) -> Result<Blender, ManagerError> {
        match self.get_blender_state_by_version(version) {
            Some(category) => {
                match category {
                    BlenderCategoryState::Loaded(blender_category) => todo!(),
                    BlenderCategoryState::NotLoaded(blender_category) => todo!(),
                }
            },
            None => todo!(),
        }
    ...
    pub(crate) fn download_blender(&mut self, version: &Version) -> Result<Blender, ManagerError> {
        ...
        let category_state = self.get_blender_state_by_version(version)
        ...
        let owned_category_state = category_state.to_owned();
        
        match owned_category_state {
            BlenderCategoryState::Loaded(loaded) => {
                loaded.get_blender(&self.download_path, version).map_err(ManagerError::Category)
            },
            BlenderCategoryState::NotLoaded(not_loaded) => todo!(),
        }
    }
