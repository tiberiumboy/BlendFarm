use crate::{services::{category::BlenderCategoryError, packages::{downloaded::Downloaded, package::PackageT}}};
use semver::Version;
use serde::{Deserialize, Serialize};
use std::{fs, io::{Error as IoError, Read}, path::{Path, PathBuf}};
use url::Url;
...
impl DownloadLink {
    pub fn new(url: Url, version: Version) -> Result<DownloadLink, BlenderCategoryError> {   
        let name = url.path_segments().ok_or(BlenderCategoryError::NotFound)?
            .last().ok_or(BlenderCategoryError::NotFound)?.to_owned();
    ...
    pub fn download(self, destination: impl AsRef<Path>) -> Result<Downloaded, IoError> {
        ...
        if !target.exists() {
            ...
            let mut response = ureq::get(self.download_url.as_str()).call().map_err(IoError::other)?;
